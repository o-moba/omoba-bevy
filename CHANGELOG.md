...
- **Demo video:** `scripts/record_demo.py` (`make demo-video`) records a short gameplay video from live practice matches: hero selection across all five classes, a shop purchase, the walk to mid (timelapsed 4×), targeting (the real cursor aims at the nearest enemy by the Tab rule: left-click locks, right-click attacks, Q/W/E/R cast, skill points spent), the Warden clearing a jungle camp and the phone controls (joystick steering and ATK/ability taps through real `TouchInput`). Captions come from the director's events; ffmpeg (with drawtext) assembles a 1280×720 H.264 file plus a manifest.
- **Trailer:** `scripts/edit_trailer.py` cuts a ~55 s music trailer from the recorded clips: shots picked by the director's caption events, 0.5 s crossfades, lower-third titles clear of the skill bar (phone titles in the letterbox band), a cold-open logo and an end card, music from a local file or composed with the ElevenLabs Music API (`--generate-music`, instrumental), loudness-normalised AAC. `trailer-manifest.json` records shots, music prompt and hashes. The README links the published trailer.
- **QA (client):** `record_qa` (`OMOBA_RECORD_DIR`, `OMOBA_RECORD_FPS`) reads the window back at a fixed rate with wall-clock timestamps and encodes PNGs off the main thread. `demo_qa` (`OMOBA_DEMO_QA_DIR`, `OMOBA_DEMO_SCRIPT=desktop|jungle|phone`, `OMOBA_DEMO_CLASS`) plays the scripted inputs; after a death or a cleared camp it walks back to the fight. `combat::nearest_enemy` exposes the Tab rule to QA builds.
- README: five classes and the current source version.
- No wire or gameplay change. Tests: client lib +2 (scene lane parsing, demo scripts); Python script tests +7 (scene declarations, demo timing).
...
- **QA:** harnesses press by `TestId` through `crate::qa::TestIdPresses` (was `NamedPresses`) and `offline_qa`, always with `SyntheticPress` (the `Interaction::Pressed` fallback is gone: every pressed control is a kit button). `TestId` no longer mirrors itself into `Name`; the phone layout pass, the edge HUD layout and the QA node dumps read `ui::test_id::NodeKey` (`TestId`, else `Name`; `crate::qa::QaName`), so dumps list kit controls under the same strings as before.
- **Behaviour change (client): top-only input for the HUD buttons.** The modal registry now gates them: while the shop is open, the gold HUD button and OPEN SHOP no longer close it (ESC and CLOSE do); while the pause menu is open, the phone bar's MENU and ? do nothing (Resume and × do); the debug HUD toggles and Retry do not react under a modal. A QA press is blocked by a modal in front, like a tap.
- **Behaviour change (client, touch):** on a phone these buttons activate on a completed tap instead of on touch-down; desktop clicks are unchanged. The debug HUD buttons now also show their on/off colour after F2/F3, and the kit repaints the skill slots and upgrade arrows while gameplay input is gated (before, only the gated handler painted them).
- No wire change. Tests: client lib 606 → 609 (586 → 589 without `qa`); server 298 (+3 ignored), shared 94, passport 26, career-store 13 (+15 ignored), account-api 6, harness 22 + 24 unchanged; Python script tests 124 (+1 skipped).

### UI kit: screens on typed actions
- Roadmap step 9b, items 3 and 4 of `docs/ui-kit.md`: the front-end screens (home, card, collection, searching, draft, loading, post-match), hero select (`team.rs`), career, social, supporter and the Combat Test panel (`sandbox/ui.rs`, completely: its text fields and teleport picking are not buttons) put `UiAction<T>` on their buttons and handle `Activated<T>` after `UiSet::Dispatch`; the help overlay's dismiss button too, so `MenuButton` could go. Career's `Action` enum is now its `UiAction` type; the hero-select handler calls `team::lock_in` for the lock-in decision.
- Removed: `ui::theme::MenuButton`, `frontend::widgets::{button, tile, compact_tile}` and their painters, the `crate::ui_theme` shim and the `frontend::widgets` palette re-export. New kit widgets `screen_button`, `screen_tile`, `compact_screen_tile`, `screen_label`; new `ButtonKind::Link` and `ButtonKind::Team(Team)` carry hero select's own blue and team colours (`ui::theme::{LINK, TEAM_GREEN, …}`).
- **Look (client):** colours, sizes and borders are unchanged; buttons are painted by `paint_pressables` from the effective interaction. Two hero-select buttons that never lit on hover now do: Back (`Secondary`, `HOVER`) and Refresh Studio (`Link`, like the wallet/account buttons next to it). Supporter buttons, card accent swatches, social wheel choices and the Combat Test value fields and toggles keep their flat look (no `ButtonStyle`).
- **Behaviour change (client, touch):** on a phone these buttons now activate on a completed tap (release inside the button, cancelled by a 10 px move), like the pause menu and career already did, instead of on touch-down; career's tab buttons join that too. Desktop clicks are unchanged. Social button presses are applied by `social_actions` later in the same frame (after the kit's dispatch) and dropped when social input is gated; career, the Combat Test panel (closed) and hero select (locked in) drop presses made while gated instead of leaving them queued.
- QA harnesses press by the same `Name`s through `crate::qa::NamedPresses` (a `SyntheticPress` for kit buttons, `Interaction::Pressed` for the rest); `sandbox/ui/qa.rs` presses by action with `SyntheticPress`. Combat Test controls gained `CombatTest…` ids and supporter buttons `Supporter…` ids.
- No wire change. Tests: client lib 586 → 606 after merging #50 (577 → 591 on its own); others unchanged.

### UI kit: scroll and modal registry
- Client (roadmap step 9b, items 1 and 2 of `docs/ui-kit.md`): one scroll system. `ui::scroll::ScrollArea` on a scrolling node says which inputs move it (wheel step per line and per pixel unit, PageUp/PageDown step, hover-only, touch-drag threshold, desktop/phone/all) and `scroll_areas` in the new `UiSet::Scroll` moves it with the touch path `mobile_ui` shipped (first finger owns the drag, nothing moves inside the threshold, window pixels converted by DPI and `UiScale`, clamped to the content). It replaces `mobile_ui::scroll_phone_panels` and `TouchScrollPanel`, `pause_menu::scroll_desktop_settings`, `career::scroll_desktop`, `social::scroll_chat`, `sandbox::ui::scroll`, `frontend::draft::scroll_panels`, `frontend::collection::scroll_collection` and the grid scrolling in `drag_to_rotate`, `team::scroll_avatar_roster`, `supporter::scroll_panel` and `edge_hud::scroll_rows`. Every panel keeps its wheel step, page step and drag threshold (table in `docs/ui-kit.md`); phone menus drag past `ui::gesture::TAP_SLOP`, the distance at which the tap recognizer cancels a tap, so a drag never presses a button.
- Client: a modal registry. `ui::modal::ModalStack` lists the open modals ordered by the z-index their roots are drawn at (shop, scoreboard, pause, career, phone server entry, supporter); `register_modal` keeps it in step with each modal's resource flag, and `input_context::register_modals` registers the six. `ModalRoot` marks each root; the tap recognizer sets the new kit-owned `Pressable::blocked` on every button outside the top modal, and scroll areas outside it do not move. The pause menu's `gate_buttons_behind_server_entry` (`Pressable::disabled` juggling) is gone: the server entry is simply on top. `input_context` reads `ModalStack::is_open()` instead of the pause, career, shop, supporter, scoreboard and server-entry flags; help, sandbox, social, front-end screens, the hero picker and phone orientation keep their own checks because they are not "a modal panel is open".
- **Behaviour change (client): top-only input.** While a registered modal is open, kit buttons (`Pressable`) and scroll areas outside it do not react: front-end screen buttons under the shell pause menu, the career entry buttons under any modal, pause buttons under the career modal (drawn in front of it), the Combat Test body, social chat log and draft/collection/hero grids under a modal. Before, a phone tap could reach the front-most `Pressable` behind a modal backdrop, and each panel scrolled under any modal.
- **Behaviour change (client): touch scrolling of the draft, loading, collection, supporter and scoreboard panels** follows the finger at any `UiScale` (the delta is converted like the phone menus; before, these moved by raw window pixels, the same only at `UiScale` 1), hit-tests with the DPI-corrected, clipped rect, and must start on the panel it scrolls (before, the supporter panel scrolled from a touch anywhere and the scoreboard from a drag that started anywhere). A second finger on the collection grid while the first turns the preview now scrolls the grid. Scrolling runs in `Update` (`UiSet::Scroll`) instead of `PostUpdate` for the pause menu, career, phone panels, social and supporter.
- No wire change. Tests: client lib 571 → 586 (566 without `qa`), shared 92 unchanged.
...
### Server-driven debug access
- **Wire-visible, additive (roadmap 11f):** `ServerPacket::Snapshot` gains `debug_access: Option<DebugAccess>` (`{"toggles":bool,"practice":bool}`), serialized only when present (`skip_serializing_if`), so every existing snapshot and the golden JSON are byte-identical and `PROTOCOL_VERSION` stays 2. `shared::debug::DebugAccess` derives serde with `serde(default)` fields. The server sends `Some(debug_access())` in every world snapshot to a joined player, including release and worker rounds (both flags false there); unjoined recipients, the prejoin status reply and the lobby snapshot carry none. Old clients ignore the field; new clients on an old server fall back to `DebugAccess::for_match_mode(match_mode)`.
- **Behaviour change (client, roadmap 11e, approved by the maintainer on 2026-09-25):** the pause menu's practice page is now a "Debug tools" page driven by that access. God mode and a new speed boost line show wherever the toggles are allowed, so a dev match gets them without `OMOBA_DEBUG_UI`; the bots and 1v1 section only where practice commands are allowed; nothing in release or worker-allocated rounds (before, a worker round showed the practice page because its mode reads `"practice"`, and the server refused the commands). In Combat Test the page offers only an entry that opens the Combat Test panel, whose actor config owns the toggles.
- **Behaviour change (client):** the 0.5 s toggle re-send runs wherever the toggles are allowed (before: only with `OMOBA_DEBUG_UI`), and both toggles reset to off locally whenever they are not allowed (leaving practice, joining a worker round or a release match, a disconnect); before, only god mode reset, and only on leaving a practice match. `OMOBA_DEBUG_UI` still enables the HUD buttons and F2/F3 hotkeys (now shown and active only where the toggles are allowed), the on-screen log and F8 debug flight.
- Tests: shared 99 → 101, server 297 → 298 (+3 ignored), client lib 567 → 573 (547 → 553 without `qa`); harness 22 + 24 unchanged; Python script tests 124 unchanged.

### Pre-beta hardening
- **Behaviour change (server, O4): unjoined endpoints on a standalone server.** A standalone server (no `OMOBA_MATCH_SERVICE` role; the LAN and beta-host shape, bound to `0.0.0.0:4000` by default) created a pre-join endpoint for every UDP source address without a limit and sent each one the full vision-filtered world snapshot every 50 ms until `PLAYER_TIMEOUT` (5 s): about 100 snapshots per datagram. Now it keeps at most `MAX_PREJOIN_ENDPOINTS` (64) unverified endpoints (neither joined nor career-authenticated); datagrams from further new addresses are dropped, with a rate-limited log line, until one joins or times out. An unverified endpoint no longer receives the world: for each datagram it sends it gets one status snapshot, at most one per `PREJOIN_STATUS_INTERVAL` (250 ms), carrying the protocol meta, map identity, match mode, `your_id`, `join_error` and the `game_state` phase with empty player and world lists (under 600 bytes). It is the existing `snapshot` packet, framed after `Hello` as before, so there is no wire change and clients (which heartbeat with `Hello` every 2 s and join right away) connect, see join rejections and join as before. Joined and career-authenticated endpoints get full snapshots as before; public roles keep their own admission and are unchanged.
- CI (O6): the `scripts` job also runs the 42 `mobile/ios` tool tests (`make test-scripts` too). The asset-gate test class no longer skips as a whole when the denied historical files are absent: 27 of its 28 tests now run in CI, and only the test that renames each denied binary skips without `OMOBA_DENIED_ASSET_ROOT` or a full clone. Its git fallback is now reachable in a full clone.
- CI (O24): a new `android` job runs `cargo check -p client --target aarch64-linux-android --locked` on every push and PR, pointing cc-rs at the runner's preinstalled NDK (the blake3, ring and oboe-sys build scripts compile C and C++ even for `check`). A new `iOS compile check` workflow runs `cargo check -p client --target aarch64-apple-ios --locked` on macOS weekly and on demand; it does not gate pull requests.
- Protocol (O10): `shared/src/protocol/wire_enums.rs` lists the 43 enums the UDP protocol carries with their variants and matches each one exhaustively without a `_` arm, pinned to `PROTOCOL_VERSION`, so a new variant does not compile until someone decides between a protocol bump and tolerant decoding. A source scan fails when a new serde enum in `shared` is neither listed there nor marked as not carried over UDP. `ARCHITECTURE.md` protocol rules state the policy and the standalone endpoint bounds.
- No wire change. Tests: server 292 → 297 (+3 ignored), shared 94 → 99, client lib 567 unchanged, harness 22 + 24 unchanged; Python script tests 96 → 124 (+1 skipped), `mobile/ios` 42 (new in CI).
...
- **Player-visible (client, O7):** the snapshot apply stage compares the draft row's loadout with `TeamSelection` before writing it, so an unchanged prematch snapshot (up to 20 a second in the draft) no longer marks it changed and rewrites the settings file. The settings file is written to `<name>.tmp`, synced and renamed over the old one, so an interrupted write can no longer truncate it and lose `client_session_id`.
- **Player-visible (client, O16):** offline practice follows the server's formulas. The "Level 6" local hero and ring bots get level-6 HP and mana pools (`max_hp_for_level`, new `shared::hero_balance::max_mana_for_level`) instead of level-1 pools; the duelist's mana pool uses its own level. Self heals, mana restores and cast damage are scaled by rank and level power as in `sim/cast.rs` (a 20-HP heal is 23.9 at level 6), mana costs by rank; dead heroes no longer regenerate mana; a dead local hero cannot cast; Q is recorded as `Attack`, like the server, via the new `PlayerActionKind::for_cast` (which the server now uses too). Harness bots are untouched.
- Server (O13): `sim/cast.rs` resolves its target with `basic_attack::resolve_hostile_target` instead of a copy of it (same rules; about 70 lines removed). The projectile `joined` check is unchanged.
- Server (O17): the round lifecycle moved out of the logging function: `settle_finished_round` finalizes a won round as Completed and starts the rematch timer, called by the tick right after `record_match_metrics` and by the career queue and play-again paths that relied on the old side effect. `restart_round` now finalizes an unsettled won round as Completed (with its winner) and anything else as Abandoned; before, a restart in the Victory state before a tick settled it recorded Abandoned.
- Server (O29): the career worker's outbox recovery is a pure `recover_outbox(&Path)` with temp-directory tests (terminal kept, live Start/Checkpoint turned into an unrated Interrupted settle carrying its allocation, rejected ids remembered, unreadable receipts left on disk). The worker is otherwise unchanged.
- Client (O25): the hero-select lock-in decision is a pure `team::lock_in(...)` returning `Ignore`, `Reconnect` or `Join { command, screen }`, with a test over every connection/mode combination; no file move.
- No wire change. Tests: server 285 → 292 (+3 ignored), client lib 562 → 567 (542 → 547 without `qa`), shared 94, harness 22 + 24 unchanged; Python script tests 96.
