...
**Status:** 15a and 15b1 are done (#39; notes in [progress/2026-09-24-client-session-events.md](../progress/2026-09-24-client-session-events.md)); 15b2, 15c and 15d are done (#41; notes in [progress/2026-09-24-client-apply-stages.md](../progress/2026-09-24-client-apply-stages.md)); 15e is done together with step 11's 11d (notes in [progress/2026-09-24-client-debug-session.md](../progress/2026-09-24-client-debug-session.md)), so step 15 is complete and 15f/15g stay optional. 15e differs from the table below in two names: the in-flight join reader is `join_in_flight()` (the `join_flow_committed` flag), and there is no `last_join()` because nothing outside `net` needs the loadout (`joined_prematch()` and `has_committed_join()` cover the readers). Differences from 15.3: `StagedSnapshot.gate` is an `ApplyOutcome` (no separate `ApplyGate` type) and `flush_session_events` is the last system of `snapshot_apply_systems()`. `SnapshotApplied.local` (`LocalHeroApply`) and the split entity stages are as designed and `SnapshotUiState` is gone. The combat round reset and the mobile clear read `RoundChanged` after `ApplySnapshot` (where it is flushed), not in `SessionReactions`; `SnapshotApplied` still has no reader outside the tests, so its `expect(dead_code)` stays until 15f.
...
Status: 15a and 15b1 done (#39, client lib 549 → 557); 15b2, 15c and 15d done (#41, 557 → 561); 15e done with 11d (561 → 562, the new test is 11d's); 15f and 15g optional.
