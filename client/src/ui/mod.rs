//! Client UI kit: one theme, one tap recognizer, typed button actions and the
//! widgets built on them. `docs/ui-kit.md` describes the API and the
...
/// runs first, typed actions are dispatched next, then the buttons repaint.
...
impl Plugin for UiKitPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.init_resource::<GestureEpoch>()
            ...
            .configure_sets(
                Update,
                (UiSet::Gesture, UiSet::Dispatch, UiSet::Paint)
                    .chain()
